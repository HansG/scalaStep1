...
When picking hello-scala's technology stack we wanted to use modern, but reasonably proven technologies. So while you
...
part of hello-scala. Also, we try to update the stack once in a while, so that it's up-to-date with current developments
and trends.

hello-scala's stack consists of the following technologies/tools, on the backend:
