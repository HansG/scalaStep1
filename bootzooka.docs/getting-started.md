...
In order to build and develop a hello-scala-derived application you'll need the following:
...
Because (as said before) hello-scala consists of two separate applications, in development you need to run both
...
hello-scala uses [PostgreSQL](https://www.postgresql.org) to store data, so you will need the database running to use the
application. By default, hello-scala uses the `bootzooka` database using the `postgres` user, connecting to a server
