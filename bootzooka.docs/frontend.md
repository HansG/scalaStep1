...
hello-scala's frontend is a true Single Page Application built with React. It can be treated as a completely separate
application or as a client for hello-scala server.
...
As a package manager, hello-scala's UI uses [Yarn](https://yarnpkg.com). Make sure to have it installed before the first
run.
...
directory). It should start your browser and point you to [hello-scala home page](http://0.0.0.0:8081/#/).
...
This task serves hello-scala application on port `8081` on `0.0.0.0` (it is available to all hosts from the same network).
